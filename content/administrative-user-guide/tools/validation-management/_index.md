...
criteria is starting with the verbiage you want to display to the end user. To do that, type in the display name; this will be what the user will see on the code summary. Keep the information concise, but also provide how they can fix the error.For example, you can display an error if a chart does not have a discharge disposition so it does not get submitted to billing without one.

Example: **Missing Discharge Disposition** - The end user would know they have to add the disposition to continue.

![](2025-02-11_ValidationMgmt6.png)
...
Upon clicking on **Add Criteria**, you will be presented with differnt types of conditions for the criteria: Account or User.

![](2025-02-03_ValidationMgmt1.png)

**Account** - Account type allows you to select fields for a patient chart/account such as discharge
date, discharge disposition, admit date, procedure codes, diagnosis codes, and many others. All of the options will appear as a dropdown in the "Filed" box.

**User** - User type allows you to restrict the rule to a certain user role(s), such as a Coder or a CDI Specialst. Typically, account-level criteria and user roles are used together. Account-level criteria can be used independently of user roles; however, creating a rule based solely on a user role without specifying any additional account-level criteria is not advisable.

Once you have defined the type of criteria (Accounr or User) and the field that you want to trigger the rule, you will be presented with the Operator and Value of the operator.

![](2025-02-11_ValidationMgmt7.png)

>[!Note]
Value is not applicable to the Operators of "Exisits" or "Does Not Exist". 
...
![](2025-02-11_ValidationMgmt8.png)
...
the first diagnosis rather we want to check all diagnoses to see if it has a status of U. **For Each** can also be
used if you want to check if a procedure code has a date of service and/or a provider attached to the procedure. We don’t want to just check the first procedure code for the date of service, we want to check each of them.

To use this field, you must *first* select the **For Each** checkbox before selecting a field. The system will open another drop-down field to the right of **For Each** to indicate where you want to create the rule. There are only a handful of fields the **For Each** field make sense for, which can be seen in the screenshot below. 

![](2025-02-11_ValidationMgmt9.png)

Once you have selected a field, you need to define the field you are looking for.
...
You can inject a field within the rule that will be displayed for the end-user. Do this by putting the *field name* in braces exactly like this {Code}. 

![](2025-02-11_ValidationMgmt11.png)

When the rule fires for the end-user, it will display the code with the issue. If there are multiple, use the **For Each** field to see each of the codes that are the issue as an individual rule.

![](2025-02-11_ValidationMgmt12.png)

## Navigate To

When your rule has been defined, you will want to decide if it makes sense to take the end user
...
page as that is where they would likely change the disposition. 

Under where you selected the level when creating a rule, there is a field for "Navigate to".

![](2025-02-11_ValidationMgmt14.png)

**Navigate to** will take the user to either the navigation pane or a place within the patient chart if the click on the error/warning. The following are the options for this function and where in the application the user will be directed to:
...
This field will have a dropdown with all available pending reasons. If a rule is triggered on an account and it has a pending reason attached, then the pending reason will be added and the user will be able to see the number of the rule. Pending reasons that have a physician or date requirement will NOT be available in this list. The
only way to remove a pending reason that was added by a rule is to have that rule cleared.

![](2025-02-11_ValidationMgmt15.png)
